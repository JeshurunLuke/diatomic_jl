...
function plotZeemanMap(mol::MoleculeHamiltonian, sol_vec; N = [0], energyRef = 0)
    ...
    for rotationalStates in indsOI,  state in (rotationalStates[1] + 1):rotationalStates[2]
        addtraces!(p, scatter(x = [sol_i.B_field*1e4 for sol_i in sol_vec], y = [sol_i.val[state] - energyRef for sol_i in sol_vec]*1e-9, text=[KetName(sol_i.vec[:, state], basisUC) for sol_i in sol_vec], name = "State $state"))
    end
...
function plotStarkMap(mol::MoleculeHamiltonian, StarkScan; N = [0], energyRef = 0)
    ...
    for rotationalStates in indsOI,  state in (rotationalStates[1] + 1):rotationalStates[2]
        ...
        addtraces!(p, scatter(x = [sol_i.E_field*1e-2 for sol_i in StarkScan], y = [sol_i.val[state] - energyRef for sol_i in StarkScan]*1e-9, text=[KetName(sol_i.vec[:, state], basisUC) for sol_i in StarkScan], name = "State $state"))
    end
